...
IF(helpsection=="modes" .OR. helpsection=="cpprop") THEN
  WRITE(*,*) "..> Mode copie de propriétés auxiliaires :"
  WRITE(*,*) "          atomsk --copy-properties <fichier1> <fichier2> [options] <outputfile> [<formats>]"
ENDIF
...
WRITE(*,*) "    gen (fichier DFTB+)     |  non   |  oui"
...
CASE(815)
  ...
  IF( ANY(DABS(reals(1:3))>0.1d0) ) THEN
    ...
    IF( ANY(DABS(reals(1:3))>9.9d0) ) THEN
      msg = "             Indices de Miller suggérés : ["//TRIM(ADJUSTL(temp))//"_"// &
          & TRIM(ADJUSTL(temp2))//"_"//TRIM(ADJUSTL(temp3))//"_"//TRIM(ADJUSTL(temp4))//"]."
    ELSE
      msg = "             Indices de Miller suggérés : ["//TRIM(ADJUSTL(temp))// &
...
CASE(2078)
  ...
  ENDIF
  IF( NINT(reals(1))==1 ) THEN
    msg = "..> 1 atome"//TRIM(temp3)//" est sélectionné."
  ELSEIF( NINT(reals(1))>1 ) THEN
    WRITE(temp,*) NINT( reals(1) )
    msg = "..> "//TRIM(ADJUSTL(temp))//" atomes sont sélectionnés"//TRIM(temp3)//"."
  ELSE
...
CASE(2149)
  ...
  IF( strings(1)=='p' .OR. strings(1)=='P' ) THEN
    msg = ">>> Réduction du système en une maille primitive..."
  ELSE
...
CASE(4073)
  ...
  msg = ">>> Copie des propriétés auxiliaires depuis "//TRIM(ADJUSTL(strings(1))) &
      & //" vers "//TRIM(ADJUSTL(strings(2)))
  CALL DISPLAY_MSG(verbosity,msg,logfile)
CASE(4074)
  ...
  WRITE(temp2,*) NINT(reals(2))
  WRITE(temp3,*) NINT(reals(3))
  msg = ">>> "//TRIM(ADJUSTL(temp))//" propriétés ont été copiées ("//TRIM(ADJUSTL(temp2))//" nouvelles, " &
      & //TRIM(ADJUSTL(temp3))//" écrasées)."
  CALL DISPLAY_MSG(verbosity,msg,logfile)
  WRITE(temp,*) NINT(reals(4))
...
CASE(4075)
  ...
  ENDDO
CASE(4076)
  msg = ">>> Association des indices de deux systèmes..."
  CALL DISPLAY_MSG(verbosity,msg,logfile)
CASE(4077)
  msg = ">>> Recherche d'atomes équivalents entre les deux systèmes..."
  CALL DISPLAY_MSG(verbosity,msg,logfile)
CASE(4078)
  ...
  WRITE(temp,*) NINT(reals(1))
  WRITE(temp2,*) NINT(reals(2))
  msg = "..> Terminé, "//TRIM(ADJUSTL(temp))//" équivalences trouvées"
  IF( NINT(reals(2))>0 ) THEN
    msg = TRIM(ADJUSTL(msg))//", "//TRIM(ADJUSTL(temp2))//" non appairés."
  ELSE
    msg = TRIM(ADJUSTL(msg))//"."
  ENDIF
  CALL DISPLAY_MSG(verbosity,msg,logfile)
CASE(4079)
  msg = "..> Marquage des nouveaux atomes..."
  CALL DISPLAY_MSG(verbosity,msg,logfile)
CASE(4080)
...
CASE(4721)
  ...
  msg = TRIM(ADJUSTL(warnmsg))//" le nombre d'atomes diffère ("//TRIM(ADJUSTL(temp))// &
      & " vs "//TRIM(ADJUSTL(temp2))//")."
  CALL DISPLAY_MSG(1,msg,logfile)
  msg = "            Les propriétés ne seront copiées que pour les "//TRIM(ADJUSTL(temp3)) &
      & //" premiers atomes."
  CALL DISPLAY_MSG(1,msg,logfile)
!
...
CASE(4833)
  ...
  WRITE(temp,*) NINT(reals(1))
  msg = TRIM(ADJUSTL(errmsg))//" apparemment VASP a été exécuté avec IBRION = "//TRIM(ADJUSTL(temp))//","
  CALL DISPLAY_MSG(1,msg,logfile)
  msg = "           donc ce fichier OUTCAR ne contient aucune configuration atomique."
  CALL DISPLAY_MSG(1,msg,logfile)
CASE(4834)
  !strings(1) = name of file
  msg = TRIM(ADJUSTL(errmsg))//" le fichier "//TRIM(ADJUSTL(strings(1)))// &
      & " ne contient aucune propriété auxiliaire, abandon."
  CALL DISPLAY_MSG(1,msg,logfile)
CASE(4835)
  msg = TRIM(ADJUSTL(errmsg))//" aucune équivalence n'a pu être trouvée entre les atomes du système 1 et ceux du système 2."
  CALL DISPLAY_MSG(1,msg,logfile)
  msg = "          Êtes-vous sûr que les deux systèmes sont du même type ?"
  CALL DISPLAY_MSG(1,msg,logfile)
CASE(4900)
